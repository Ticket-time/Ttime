...
exports.apply = async (req, res) => {
  ...
  //db에 추가
  db.query(
    "insert into apply(showid, userid) values(?, ?)",
    [showid, userid],
    (err, rows) => {
      if (err) {
        // 잘못된 정보 입력 or db 문제
...
exports.showDetails = async (req, res) => {
  ...
  db.query(sql, [showid], (err, rows) => {
    ...
    db.query(sql, [userid, showid], (err, rows) => {
      if (err) {
        console.log(err);
        throw err;
      }
      ...
      else return res.send({ success: true, code: 333, message: "응모 가능" });
    });
  });
};
